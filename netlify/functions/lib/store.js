...
async function atomicUpdateJSON(key, initialValue, updater, options = {}) {
  ...
  const retries = Math.max(1, Math.min(24, Number(options.retries || 16)));
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    // Small jitter prevents a burst of concurrent requests from repeatedly
    // colliding on the same ETag.
    await new Promise(resolve => setTimeout(resolve, Math.min(40 + Math.random() * 80, 120)));
  }
